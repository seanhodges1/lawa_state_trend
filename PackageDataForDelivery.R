...
trends<-read.csv(file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/trend_sites_catchment_region.csv",stringsAsFactors = FALSE)
...
StartMonth <- 1
EndMonth <- 12
load(file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/trenddata2005-2014.RData")
...
df_minfreq_Catchment <- summaryBy(Value~Region+Catchment,data=df_minfreq_length,
                               FUN=c(median), keep.name=TRUE)

# Determine annual medians counts by region
df_minfreq_Region <- summaryBy(Value~Region,data=df_minfreq_length,
                                         FUN=c(median), keep.name=TRUE)
...
tmp1<-tmp1[,c(2,3,4,5,6,12,13,10)]
...
write.csv(tmp1,file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/trend_tmp1.csv")         ## 
...
state<-read.csv(file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/LAWA_STATE_FINAL_2010-2014.csv",stringsAsFactors = FALSE)
...
dl<-read.csv(file="//file/herman/R/OA/08/02/2015/Water Quality/RScript/lawa_state/AgencyDLs.csv",stringsAsFactors = FALSE)
...
# ---------------------
# Export to XL workbook
write.csv(state,file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/state_tmp1.csv",row.names = FALSE)         ## 
...
load("//file/herman/R/OA/08/02/2015/Water Quality/ROutput/trenddata2005-2014.RData",verbose=TRUE)
raw<-lawadata[,c(1,5,2,2,13,4,7,15,18,20,21,26,27,22,33,16)]
rm(lawadata)
...
c <- c("Site",	"Measurement",	"DateTime",	"Date",	"Original.Value", "Method",	"CenType",	"LAWAID", "Region",	"Landuse",	"Altitude",	"WSGS84_X",	"WSGS84_Y",	"Catchment",	
       "CatchmentNameLAWA",	"Agency",	"Symbol","Raw.Value","QC", "License",	"Ownership",	"Disclaimer")
names(raw) <- c
...
# ---------------------
# Export to XL workbook
write.csv(raw,file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/rawdata_all.csv",row.names = FALSE)         ## 
write.csv(raw_sans,file="//file/herman/R/OA/08/02/2015/Water Quality/ROutput/rawdata_minus_niwa_ccc.csv",row.names = FALSE)         ## 
